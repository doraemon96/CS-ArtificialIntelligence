...
def depthFirstSearch(problem):
    ...
    node_list = Stack()
    return search(problem, node_list)
...
def uniformCostSearch(problem):
    ...
    """problem.costFn = lambda x: ..."""

    initial_state = problem.getStartState()
    initial_actions = []
    initial_candidate = (initial_state, initial_actions,1)
    fringe.push(initial_candidate,1)
    closed_set = set()
    while not fringe.isEmpty():
        candidate = fringe.pop()
    ...
    while not fringe.isEmpty():
        ...
        if state not in closed_set:
            ...
            candidate_successors = map(lambda x: (x[0], actions + [x[1]],
                cost+x[2], heuristic(x[0],problem)), candidate_successors)
            for candidate in candidate_successors:
                fringe.push((candidate[0],candidate[1],candidate[2]),
